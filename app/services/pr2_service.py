...
# Import functions via absolute path
...
async def _get_transcription_by_id(tid: str):
    db = get_database()
    if not db: return None
    try:
        return await db.transcriptions.find_one({"_id": ObjectId(tid)})
    except:
        return None

async def _get_soap_note_by_transcription_id(tid: str):
    db = get_database()
    if not db: return None
    # Assuming soap_notes collection is 'soap_notes'
    return await db.soap_notes.find_one({"transcription_id": tid})
...
async def create_pr2_form_service(data: PR2Form):
    """
...
async def generate_pr2_from_transcription_service(
    transcription_id: Optional[str] = None,
    transcription: Optional[str] = None,
    use_latest_intake: bool = False,
    use_latest_followup: bool = False,
    pr1_id: Optional[str] = None
):
    """
    ...
    **Returns:**
    ...
    try:
        ...
        if transcription_id:
            transcription_doc = await _get_transcription_by_id(transcription_id)
        ...
        soap_doc = None
        if transcription_id:
            soap_doc = await _get_soap_note_by_transcription_id(transcription_id)
...
async def generate_pr2_from_soap_service(
    soap_id: str,
    use_latest_intake: bool = False,
    use_latest_followup: bool = False,
    pr1_id: Optional[str] = None
...
async def get_latest_pr2_form_service():
    """
...
async def get_pr2_form_by_id_service(form_id: str):
    """
...
async def get_pr2_form_by_soap_id_service(soap_id: str):
    """
